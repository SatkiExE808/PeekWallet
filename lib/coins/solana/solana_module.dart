// Solana CoinModule. Supports:
//   - bip39_12 / bip39_24 create + restore. Same BIP39 mnemonic
//     model as Bitcoin/Ethereum; the difference is SLIP-0010 ed25519
//     derivation (vs BIP32 secp256k1) under the hood.
//
...
class SolanaModule implements CoinModule {
  const SolanaModule();
  ...
  @override
  Color get color => const Color(0xFF9945FF);
  @override
    ...
    switch (format) {
      ...
      default:
        throw CoinModuleError(
            'Solana doesn\'t support ${format.displayName} for create');
    }
  ...
  Future<RestoredWalletMaterial> restoreFrom({
    ...
    if (format != SeedFormat.bip39_12 && format != SeedFormat.bip39_24) {
      throw CoinModuleError(
          'Solana doesn\'t support ${format.displayName} for restore');
    }
    ...
    final addr = await deriveSolanaAddress(
        mnemonic: normalised, passphrase: passphrase);
